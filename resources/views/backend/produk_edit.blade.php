...
<div class="card">
    <div class="card-body">
        <h6 class="mb-4 text-uppercase">Edit Produk</h6>
        <form class="form" action="{{ route('produk.update') }}" method="post" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group mb-4">
                ...
                <input type="text" class="form-control @error('stok_produk') is-invalid @enderror"  name="stok_produk" value="<?= old('stok_produk', $produk->stok_produk) ?>">
                <div class="invalid-feedback">
                    @error('stok_produk')
                    <div class="text-danger">{{ $message }}</div>
                    @enderror
            ...
            <div class="form-group mb-4">
                ...
                <select type="text" class="form-control @error('kategori_id') is-invalid @enderror"  name="kategori_id">
                    ...
                    @foreach ($kategori as $row)
                        <option value="{{$row->kategori_id}}" {{ ($row->kategori_id == old('kategori_id', $produk->kategori_id)) ? 'selected' : '' }}>{{ $row->nama_kategori }}</option>
                    @endforeach
                </select>
                <div class="invalid-feedback">
                    @error('kategori_id')
            ...
            <div class="form-group mb-4">
                ...
                <select type="text" class="form-control @error('penenun_id') is-invalid @enderror"  name="penenun_id">
                    ...
                    @foreach ($penenun as $row)
                        <option value="{{$row->penenun_id}}" {{ ($row->penenun_id == old('penenun_id', $produk->penenun_id)) ? 'selected' : '' }}>{{ $row->nama_penenun }}</option>
                    @endforeach
                </select>
                <div class="invalid-feedback">
                    @error('penenun_id')
            ...
            <div class="form-group mb-4">
                ...
                <textarea type="text" class="form-control @error('deskripsi_produk') is-invalid @enderror"  name="deskripsi_produk">{{ old('deskripsi_produk', $produk->deskripsi_produk) }}</textarea>
                <div class="invalid-feedback">
            ...
            <div class="form-group mb-2">
                <img src="{{ asset('storage/'.$produk->gambar_produk) }}" alt="Gambar Produk" class="img img-thumbnail">
            </div>
            <div class="form-group mb-4">
                <label for="gambar_produk">Gambar Produk <span class="text-warning small">Biarkan kosong jika tidak ingin mengganti</span></label>
